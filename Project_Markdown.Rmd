...
```{r plotting,fig.width=8, fig.height=4.5}
par(mfrow = c(1, 2))
par(cex.axis = 0.7)
...
```{r plotting_norm_vs_lambda, fig.width=4.5, fig.height=3.5, fig.align='center'}
library(latex2exp)
...
plot(L, rat, type="l", log="x", col="darkblue", lwd=1.5,
     xlab = TeX("$\\lambda$"), 
     ylab = TeX("$\\frac{||\\hat{\\beta}^R_\\lambda||_2}{||\\hat{\\beta}^{OLS}||_2}$"),
     main = "Shrinkage Profile", las=1, cex.lab=1.2)
grid()
```

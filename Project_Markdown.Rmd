...
PCA turns variables that use different scales and are correlated into a smaller set of new variables that all use the same scale, are uncorrelated. PCR then predicts Balance using these standardized super-variables, and cross-validation assesses model fit.:
...
```{r plotting,fig.width=8, fig.height=3.5}
par(mfrow = c(1, 2))
par(cex.axis = 0.7)
...
matplot(x_grid, stretched_coefs, type = 'l', col = line_cols, lty = line_types, 
        lwd = line_widths, xlab = "Number of Components", ylab = 
          "Standardized Coefficients", cex.lab = 0.8)
legend("topleft", names(Credit)[c(1, 2, 3, 8)], col = c("black", "red", "blue",
      "orange"),  lty = 1:4, lwd = 1, bty = "n", cex = 0.6)
...
#$val$ (1-> Cross Validation SE, 1 -> Balance, -1 -> Remove Intercept )
plot(cv_mse, col = "purple", type = "b", ylab = "Cross-Validation MSE", 
     xlab = "Number of Components", cex.lab = 0.8)
```
...
Lambda $\lambda$ is the tuning parameter defined in shrinkage regression (Ridge), which serves to artificially minimize (shrink) the coefficients ($\beta$) Ridge Regression Formula, the general Ridge regression objective is:
...
To find the minimum, we differentiate the term inside the brackets with respect to $\beta_j$: $$ \frac{\partial\mathcal{L}}{\partial \beta_j} \left( (y_j - \beta_j)^2 + \lambda \beta_j^2 \right) = -2(y_j - \beta_j) + 2\lambda\beta_j = 0 $$ $$-2y_{j} + 2(1+\lambda)\beta_{j} = 0$$

Rearranging for $\beta_j$ yields the Ridge estimator for this special case: $$\hat{\beta}_{j,\lambda}^{R} = \frac{y_{j}}{1+\lambda} \quad \quad \quad (Formula \quad 2.9) $$
...
plot(L, rat, type="l", log="x", col="darkblue", lwd=1.5,
     xlab = expression(lambda), ylab = "", main = "Shrinkage Profile",las = 1, cex.main=0.8)
...
Vector Form: Stacking the coefficients into a vector after rearranging Formula 2.9 yields: $$\hat{\beta}_{\lambda}^{R} = \frac{1}{1+\lambda}* y$$

Final $l_2$ Norm on both sides - Taking the square root uses the standard identity $||cv||_{2} = |c| \cdot ||v||_{2}$:$$||\hat{\beta}_{\lambda}^{R}||_{2} = \frac{1}{1+\lambda}\sqrt{\sum_{j=1}^{p}y_{j}^{2}}$$

$$||\hat{\beta}_{\lambda}^{R}||_{2} = \frac{1}{1+\lambda}||y||_{2} \quad \quad \quad (Formula \quad 2.12) $$
